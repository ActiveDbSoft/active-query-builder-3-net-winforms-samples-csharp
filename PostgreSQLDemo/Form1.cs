...
namespace PostgreSQLDemo
{
    public partial class Form1 : Form
    {
        ...
        private void connectMetadataToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (PostgreSQLConnectionForm f = new PostgreSQLConnectionForm())
            {
                if (f.ShowDialog() == DialogResult.OK)
                {
                    // create new SqlConnection object using the connections string from the connection form
                    postgreSQLMetadataProvider1.Connection = new NpgsqlConnection(f.ConnectionString);

                    // setup the query builder with metadata and syntax providers
                    queryBuilder1.SyntaxProvider = postgreSQLSyntaxProvider1;
                    queryBuilder1.MetadataProvider = postgreSQLMetadataProvider1;
        ...
        private void tabControl1_Selected(object sender, TabControlEventArgs e)
        {
            ...
            if (e.TabPage == tabPageData)
            {
                ...
                {
                    NpgsqlCommand command = (NpgsqlCommand) queryBuilder1.MetadataProvider.Connection.CreateCommand();
